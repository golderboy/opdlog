object active_log_form: Tactive_log_form
  Left = 0
  Top = 0
  Caption = 'active_log_form'
  ClientHeight = 522
  ClientWidth = 779
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  WindowState = wsMaximized
  DesignSize = (
    779
    522)
  PixelsPerInch = 96
  TextHeight = 13
  object header_opdlog: TJvNavPanelHeader
    Left = 0
    Top = 0
    Width = 779
    Height = 49
    Align = alTop
    Caption = ' Active Log'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    ImageIndex = 0
  end
  object Panel2: TPanel
    Left = 0
    Top = 49
    Width = 779
    Height = 41
    Align = alTop
    TabOrder = 1
    DesignSize = (
      779
      41)
    object cxLabel2: TcxLabel
      Left = 40
      Top = 18
      Caption = #3588#3657#3609#3627#3634' HN / PID'
    end
    object post_date_btn: TRzBitBtn
      Left = 588
      Top = 13
      Width = 107
      Anchors = [akTop, akRight]
      Caption = 'Preview'
      TabOrder = 1
      OnClick = post_date_btnClick
      Glyph.Data = {
        
        E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
      NumGlyphs = 2
    end
    object excel_export_btn: TRzBitBtn
      Left = 701
      Top = 13
      Anchors = [akTop, akRight]
      Caption = 'EXCEL'
      TabOrder = 2
      OnClick = excel_export_btnClick
      Glyph.Data = {
        
        E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
      NumGlyphs = 2
    end
    object key_form: TcxTextEdit
      Left = 120
      Top = 17
      Properties.ReadOnly = False
      TabOrder = 3
      Width = 401
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 495
    Width = 779
    Height = 27
    Align = alBottom
    TabOrder = 2
    DesignSize = (
      779
      27)
    object RzBitBtn1: TRzBitBtn
      Left = 701
      Top = 3
      Anchors = [akRight, akBottom]
      Caption = 'Exit'
      TabOrder = 0
      OnClick = RzBitBtn1Click
      Glyph.Data = {
        
        57E8E8E8E8E8E8E8E8E8E8E8E8AC818181E8E8E8E8E8E8E8E8E8}
      NumGlyphs = 2
    end
    object cxLabel1: TcxLabel
      Left = 40
      Top = 3
      Caption = #3592#3635#3609#3623#3609
    end
    object record_count: TcxLabel
      Left = 95
      Top = 3
      Caption = '0'
    end
  end
  object show_visit: TcxGrid
    AlignWithMargins = True
    Left = 3
    Top = 93
    Width = 773
    Height = 399
    Align = alClient
    Font.Charset = THAI_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 3
    object show_visitDBTableView1: TcxGridDBTableView
      Navigator.Buttons.CustomButtons = <>
      DataController.DataSource = Dactive_log
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <>
      DataController.Summary.SummaryGroups = <>
      Filtering.ColumnFilteredItemsList = True
      OptionsData.Appending = True
      OptionsView.GroupByBox = False
      object show_visitDBTableView1officer_activity_log_datetime: TcxGridDBColumn
        Caption = 'datetime'
        DataBinding.FieldName = 'officer_activity_log_datetime'
        Width = 118
      end
      object show_visitDBTableView1officer_activity_log_computer: TcxGridDBColumn
        Caption = 'computer'
        DataBinding.FieldName = 'officer_activity_log_computer'
        Width = 62
      end
      object show_visitDBTableView1officer_activity_log_table: TcxGridDBColumn
        Caption = 'log_table'
        DataBinding.FieldName = 'officer_activity_log_table'
        Width = 62
      end
      object show_visitDBTableView1officer_activity_log_operation: TcxGridDBColumn
        Caption = 'activity_log'
        DataBinding.FieldName = 'officer_activity_log_operation'
        Width = 73
      end
      object show_visitDBTableView1name: TcxGridDBColumn
        DataBinding.FieldName = 'name'
        Width = 97
      end
      object show_visitDBTableView1officer_activity_log_parent_kv: TcxGridDBColumn
        Caption = 'parent_hn'
        DataBinding.FieldName = 'officer_activity_log_parent_kv'
        Width = 69
      end
      object show_visitDBTableView1active_screen_class: TcxGridDBColumn
        Caption = 'active_screen'
        DataBinding.FieldName = 'active_screen_class'
        Width = 150
      end
      object show_visitDBTableView1log_field_change: TcxGridDBColumn
        DataBinding.FieldName = 'log_field_change'
        Width = 628
      end
    end
    object show_visitLevel1: TcxGridLevel
      GridView = show_visitDBTableView1
    end
  end
  object process_Qrb1k: TcxProgressBar
    Left = 40
    Top = 288
    Anchors = [akLeft, akTop, akRight, akBottom]
    TabOrder = 4
    Visible = False
    Width = 689
  end
  object Qactive_log: TMyQuery
    SQLInsert.Strings = (
      'INSERT INTO opdscreen'
      
        '  (vn, hn, vstdate, vsttime, bpd, bps, bw, cc, hr, pe, pulse, te' +
        'mperature, rr, height, bmi, hpi, pmh, waist)'
      'VALUES'
      
        '  (:vn, :hn, :vstdate, :vsttime, :bpd, :bps, :bw, :cc, :hr, :pe,' +
        ' :pulse, :temperature, :rr, :height, :bmi, :hpi, :pmh, :waist)')
    Connection = db_connect_m.connect_db
    SQL.Strings = (
      
        'SELECT l.officer_activity_log_datetime,l.officer_activity_log_co' +
        'mputer,l.officer_activity_log_table,l.officer_activity_log_opera' +
        'tion'
      
        ',o.`name`,l.officer_activity_log_parent_kv,l.active_screen_class' +
        ',l.log_field_change'
      ''
      ''
      'from officer_activity_log  l'
      'LEFT OUTER JOIN opduser  o on o.loginname = l.staff'
      'limit 1;')
    ReadOnly = True
    Options.AutoRefresh = True
    Options.AutoRefreshInterval = 120
    Left = 320
    Top = 168
  end
  object Dactive_log: TMyDataSource
    AutoEdit = False
    DataSet = Qactive_log
    Left = 424
    Top = 168
  end
end
