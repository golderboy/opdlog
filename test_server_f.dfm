object test_connect: Ttest_connect
  Left = 0
  Top = 0
  Caption = 'test_connect'
  ClientHeight = 482
  ClientWidth = 551
  Color = clBtnFace
  Font.Charset = THAI_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'MS Sans Serif'
  Font.Style = [fsBold]
  OldCreateOrder = False
  OnShow = FormShow
  DesignSize = (
    551
    482)
  PixelsPerInch = 96
  TextHeight = 20
  object cxLabel1: TcxLabel
    Left = 152
    Top = 8
    Caption = #3626#3606#3634#3609#3632#3585#3634#3619#3648#3594#3639#3656#3629#3617#3605#3656#3629
  end
  object cxLabel2: TcxLabel
    Left = 72
    Top = 48
    Caption = 'SERVER '#3627#3621#3633#3585
  end
  object cxLabel3: TcxLabel
    Left = 344
    Top = 48
    Caption = 'SERVER '#3626#3635#3619#3629#3591
  end
  object cxLabel4: TcxLabel
    Left = 24
    Top = 88
    Caption = #3585#3634#3619#3648#3594#3639#3656#3629#3617#3605#3656#3629
  end
  object cxLabel5: TcxLabel
    Left = 280
    Top = 88
    Caption = #3585#3634#3619#3648#3594#3639#3656#3629#3617#3605#3656#3629
  end
  object cxLabel6: TcxLabel
    Left = 24
    Top = 118
    Caption = #3592#3635#3609#3623#3609' VN'
  end
  object cxLabel7: TcxLabel
    Left = 24
    Top = 178
    Caption = #3592#3635#3609#3623#3609' AN'
  end
  object cxLabel8: TcxLabel
    Left = 280
    Top = 118
    Caption = #3592#3635#3609#3623#3609' VN'
  end
  object cxLabel9: TcxLabel
    Left = 280
    Top = 178
    Caption = #3592#3635#3609#3623#3609' AN'
  end
  object exit_btn: TRzBitBtn
    Left = 389
    Top = 417
    Width = 154
    Height = 57
    Anchors = [akRight, akBottom]
    Caption = 'Exit'
    TabOrder = 9
    OnClick = exit_btnClick
    Glyph.Data = {
      
      57E8E8E8E8E8E8E8E8E8E8E8E8AC818181E8E8E8E8E8E8E8E8E8}
    NumGlyphs = 2
  end
  object sta_tmain: TcxLabel
    Left = 117
    Top = 88
    AutoSize = False
    Height = 24
    Width = 68
  end
  object sta_tslave: TcxLabel
    Left = 389
    Top = 88
    AutoSize = False
    Height = 24
    Width = 68
  end
  object cxLabel10: TcxLabel
    Left = 24
    Top = 148
    Caption = 'Last VN'
  end
  object cxLabel11: TcxLabel
    Left = 280
    Top = 148
    Caption = 'Last VN'
  end
  object cxLabel14: TcxLabel
    Left = 24
    Top = 208
    Caption = 'Last AN'
  end
  object cxLabel15: TcxLabel
    Left = 280
    Top = 208
    Caption = 'Last AN'
  end
  object cxDBLabel1: TcxDBLabel
    Left = 109
    Top = 118
    DataBinding.DataField = 'sum_vn'
    DataBinding.DataSource = Dsum_vn_main
    Height = 21
    Width = 140
  end
  object cxDBLabel2: TcxDBLabel
    Left = 109
    Top = 151
    DataBinding.DataField = 'last_vn'
    DataBinding.DataSource = Dsum_vn_main
    Height = 21
    Width = 140
  end
  object cxDBLabel3: TcxDBLabel
    Left = 109
    Top = 178
    DataBinding.DataField = 'sum_an'
    DataBinding.DataSource = Dsum_vn_main
    Height = 21
    Width = 140
  end
  object cxDBLabel4: TcxDBLabel
    Left = 109
    Top = 208
    DataBinding.DataField = 'last_an'
    DataBinding.DataSource = Dsum_vn_main
    Height = 21
    Width = 140
  end
  object cxDBLabel5: TcxDBLabel
    Left = 389
    Top = 118
    DataBinding.DataField = 'sum_vn'
    DataBinding.DataSource = Dsum_vn_slave
    Height = 21
    Width = 140
  end
  object cxDBLabel6: TcxDBLabel
    Left = 389
    Top = 151
    DataBinding.DataField = 'last_vn'
    DataBinding.DataSource = Dsum_vn_slave
    Height = 21
    Width = 140
  end
  object cxDBLabel7: TcxDBLabel
    Left = 389
    Top = 178
    DataBinding.DataField = 'sum_an'
    DataBinding.DataSource = Dsum_vn_slave
    Height = 21
    Width = 140
  end
  object cxDBLabel8: TcxDBLabel
    Left = 389
    Top = 208
    DataBinding.DataField = 'last_an'
    DataBinding.DataSource = Dsum_vn_slave
    Height = 21
    Width = 140
  end
  object cxLabel12: TcxLabel
    Left = 24
    Top = 235
    Caption = 'Last HN'
  end
  object cxDBLabel9: TcxDBLabel
    Left = 109
    Top = 235
    DataBinding.DataField = 'last_hn'
    DataBinding.DataSource = Dsum_vn_main
    Height = 21
    Width = 140
  end
  object cxLabel13: TcxLabel
    Left = 279
    Top = 235
    Caption = 'Last HN'
  end
  object cxDBLabel10: TcxDBLabel
    Left = 389
    Top = 235
    DataBinding.DataField = 'last_hn'
    DataBinding.DataSource = Dsum_vn_slave
    Height = 21
    Width = 140
  end
  object RzBitBtn1: TRzBitBtn
    Left = 24
    Top = 417
    Width = 149
    Height = 57
    Anchors = [akLeft, akBottom]
    Caption = 'Execute'
    TabOrder = 28
    OnClick = RzBitBtn1Click
    Glyph.Data = {
      
      AAAAAAAAAAAAAAAAE8E8E8E8E8E8E8818181818181818181E8E8}
    NumGlyphs = 2
  end
  object date_chack_text: TcxTextEdit
    Left = 24
    Top = 265
    Properties.ReadOnly = True
    TabOrder = 29
    Text = 'date_chack_text'
    Visible = False
    Width = 505
  end
  object process_Qrb1k: TcxProgressBar
    Left = 24
    Top = 299
    Anchors = [akLeft, akTop, akRight, akBottom]
    TabOrder = 30
    Visible = False
    Width = 505
  end
  object process_Qrb2k: TcxProgressBar
    Left = 24
    Top = 333
    Anchors = [akLeft, akTop, akRight, akBottom]
    TabOrder = 31
    Visible = False
    Width = 505
  end
  object Qsum_vn_main: TMyQuery
    Connection = db_connect_m.connect_db
    SQL.Strings = (
      'select vn,'
      'count(vn) as sum_vn,'
      'count(an) as sum_an,'
      
        '(select vn from ovst where vn <> '#39#39' AND vstdate between :date_te' +
        'xt AND :date_text order by vn desc limit 1) as last_vn,'
      
        '(select an from ovst where an <> '#39#39' AND vstdate between :date_te' +
        'xt AND :date_text order by an desc limit 1) as last_an,'
      
        '(select hn from ovst where hn <> '#39#39' and hn < '#39'199999999'#39' AND vst' +
        'date between :date_text AND :date_text order by hn desc limit 1)' +
        ' as last_hn'
      'from ovst'
      'limit 1'
      ';')
    Left = 80
    Top = 312
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'date_text'
      end
      item
        DataType = ftUnknown
        Name = 'date_text'
      end
      item
        DataType = ftUnknown
        Name = 'date_text'
      end
      item
        DataType = ftUnknown
        Name = 'date_text'
      end
      item
        DataType = ftUnknown
        Name = 'date_text'
      end
      item
        DataType = ftUnknown
        Name = 'date_text'
      end>
  end
  object Dsum_vn_main: TMyDataSource
    DataSet = Qsum_vn_main
    Left = 176
    Top = 312
  end
  object Qsum_vn_slave: TMyQuery
    Connection = db_connect_m.connect_slave
    SQL.Strings = (
      'select vn,'
      'count(vn) as sum_vn,'
      'count(an) as sum_an,'
      
        '(select vn from ovst where vn <> '#39#39' AND vstdate between :date_te' +
        'xt AND :date_text order by vn desc limit 1) as last_vn,'
      
        '(select an from ovst where an <> '#39#39' AND vstdate between :date_te' +
        'xt AND :date_text order by an desc limit 1) as last_an,'
      
        '(select hn from ovst where hn <> '#39#39' and hn < '#39'199999999'#39' AND vst' +
        'date between :date_text AND :date_text order by hn desc limit 1)' +
        ' as last_hn'
      'from ovst'
      'limit 1'
      ';')
    Left = 320
    Top = 312
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'date_text'
      end
      item
        DataType = ftUnknown
        Name = 'date_text'
      end
      item
        DataType = ftUnknown
        Name = 'date_text'
      end
      item
        DataType = ftUnknown
        Name = 'date_text'
      end
      item
        DataType = ftUnknown
        Name = 'date_text'
      end
      item
        DataType = ftUnknown
        Name = 'date_text'
      end>
  end
  object Dsum_vn_slave: TMyDataSource
    DataSet = Qsum_vn_slave
    Left = 416
    Top = 312
  end
end
